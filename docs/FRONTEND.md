...
- Под полями `text` отображается «черновой предпросмотр» введённого Markdown‑текста, который рендерится через `react-markdown` (списки, заголовки, ссылки и пр.) в лёгком виде.
